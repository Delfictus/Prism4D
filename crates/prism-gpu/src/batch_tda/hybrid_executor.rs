...
impl HybridTdaExecutor {
    ...
    fn execute_gpu(&mut self, neighborhood: &NeighborhoodData) -> Result<Vec<f32>, PrismError> {
        ...
        // Get mutable reference to buffers for upload
        {
            ...
            // Upload data
            stream.memcpy_htod(&neighborhood.offsets, &mut buffers.d_offsets)
                .map_err(|e| PrismError::gpu("hybrid_executor", format!("{:?}", e)))?;
            stream.memcpy_htod(&neighborhood.neighbor_indices, &mut buffers.d_neighbor_indices)
                .map_err(|e| PrismError::gpu("hybrid_executor", format!("{:?}", e)))?;
            stream.memcpy_htod(&neighborhood.neighbor_distances, &mut buffers.d_neighbor_distances)
                .map_err(|e| PrismError::gpu("hybrid_executor", format!("{:?}", e)))?;
            stream.memcpy_htod(&neighborhood.neighbor_coords, &mut buffers.d_neighbor_coords)
                .map_err(|e| PrismError::gpu("hybrid_executor", format!("{:?}", e)))?;
            stream.memcpy_htod(&neighborhood.center_coords, &mut buffers.d_center_coords)
                .map_err(|e| PrismError::gpu("hybrid_executor", format!("{:?}", e)))?;
        }
        ...
        unsafe {
            ...
            builder.arg(&buffers.d_features);
