...
impl StreamingTdaPipeline {
    ...
    pub fn new(ctx: Arc<CudaContext>, ptx_path: &Path) -> Result<Self, PrismError> {
        ...
        let kernel = module.load_function("hybrid_tda_kernel")
            .map_err(|e| PrismError::config(format!("Get kernel: {:?}", e)))?;
    ...
    fn execute_single(&mut self, neighborhood: &NeighborhoodData) -> Result<Vec<f32>, PrismError> {
        ...
        let gpu_active = self.buffers.gpu_active;
        {
            ...
            // Upload data
            self.compute_stream.memcpy_htod(&neighborhood.offsets, &mut buffer.d_offsets)
                .map_err(|e| PrismError::gpu("streaming", format!("{:?}", e)))?;
            self.compute_stream.memcpy_htod(&neighborhood.neighbor_indices, &mut buffer.d_neighbor_indices)
                .map_err(|e| PrismError::gpu("streaming", format!("{:?}", e)))?;
            self.compute_stream.memcpy_htod(&neighborhood.neighbor_distances, &mut buffer.d_neighbor_distances)
                .map_err(|e| PrismError::gpu("streaming", format!("{:?}", e)))?;
            self.compute_stream.memcpy_htod(&neighborhood.neighbor_coords, &mut buffer.d_neighbor_coords)
                .map_err(|e| PrismError::gpu("streaming", format!("{:?}", e)))?;
            self.compute_stream.memcpy_htod(&neighborhood.center_coords, &mut buffer.d_center_coords)
                .map_err(|e| PrismError::gpu("streaming", format!("{:?}", e)))?;
        }
        ...
        unsafe {
            let mut builder = self.compute_stream.launch_builder(&self.kernel);
            builder.arg(&buffer.d_offsets);
            builder.arg(&buffer.d_neighbor_indices);
            builder.arg(&buffer.d_neighbor_distances);
            builder.arg(&buffer.d_neighbor_coords);
            builder.arg(&buffer.d_center_coords);
            builder.arg(&buffer.d_features);
            builder.arg(&n_residues_u32);
            builder.arg(&n_radii_u32);
            builder.arg(&self.config.scales[0]);
            builder.arg(&self.config.scales[1]);
            builder.arg(&self.config.scales[2]);
            builder.arg(&self.config.scales[3]);
            builder.launch(cfg)
                .map_err(|e| PrismError::gpu("streaming", format!("{:?}", e)))?;
        }
