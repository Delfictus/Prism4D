...
impl CmaOptimizer {
    ...
    pub fn new(device: Arc<CudaContext>, dimensions: usize, seed: u64) -> Result<Self> {
        ...
        // Initialize sigma
        stream.memcpy_htod(&vec![params.sigma], &mut d_sigma)?;
        stream.memcpy_htod(&vec![f32::INFINITY], &mut d_best_fitness)?;
        stream.memcpy_htod(&vec![1.0f32], &mut d_condition)?;
        ...
        stream.memcpy_htod(&weights, &mut d_weights)?;
        ...
        stream.memcpy_htod(&identity, &mut d_covariance)?;
        stream.memcpy_htod(&identity, &mut d_bd_matrix)?;
