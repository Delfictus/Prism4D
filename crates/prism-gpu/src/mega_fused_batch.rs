...
impl MegaFusedBatchGpu {
    ...
    pub fn new(context: Arc<CudaContext>, ptx_dir: &Path) -> Result<Self, PrismError> {
        ...
        let training_func = if batch_path.exists() {
            ...
            match module.load_function("mega_fused_batch_training") {
                Ok(func) => {
                    log::info!("Loaded mega_fused_batch_training (reservoir state export kernel)");
                    Some(func)
                }
                Err(e) => {
    ...
    pub fn detect_pockets_batch(
        ...
        if !batch.frequencies_packed.is_empty() {
            ...
            self.stream.memcpy_htod(frequencies_to_copy, d_frequencies)
                .map_err(|e| PrismError::gpu("mega_fused_batch", format!("Copy frequencies: {}", e)))?;
            self.stream.memcpy_htod(velocities_to_copy, d_velocities)
        ...
        if !batch.p_neut_time_series_75pk_packed.is_empty() {
            if let Some(d_p_neut_75pk) = self.buffer_pool.d_p_neut_time_series_75pk.as_mut() {
                ...
                self.stream.memcpy_htod(&batch.p_neut_time_series_75pk_packed, d_p_neut_75pk)
        ...
        if !batch.current_immunity_levels_75_packed.is_empty() {
            if let Some(d_immunity_75) = self.buffer_pool.d_current_immunity_levels_75.as_mut() {
                ...
                self.stream.memcpy_htod(immunity_to_copy, d_immunity_75)
        ...
        if !batch.pk_params_packed.is_empty() {
            if let Some(d_pk_params) = self.buffer_pool.d_pk_params.as_mut() {
                self.stream.memcpy_htod(&batch.pk_params_packed, d_pk_params)
        ...
        unsafe {
            ...
            builder.arg(d_residue_types);  // PRISM>4D
            // Structure descriptors
            ...
            builder.arg(self.buffer_pool.d_combined_features.as_ref().unwrap());  // PRISM>4D: 136-dim features
            ...
            builder.arg(d_frequencies);  // Per-structure frequencies
            builder.arg(d_velocities);   // Per-structure velocities

            // Stage 9-10: Immunity inputs with 75-PK support (FIX #2 CORRECTED)
            let null_ptr: u64 = 0;

            //=== PASS REAL EPITOPE ESCAPE (PER-RESIDUE LAYOUT, NOT nullptr) ===
            if let Some(d_epitope_escape) = self.buffer_pool.d_epitope_escape.as_ref() {
                eprintln!("[DMS GPU] Passing real epitope escape buffer ({} values)", batch.epitope_escape_packed.len());
                builder.arg(d_epitope_escape);  // REAL DMS DATA
            } else {
                builder.arg(&null_ptr);  // Fallback
            }

            // Immunity events (still nullptr - not yet implemented)
            ...
            // P_neut time series: Use 75-PK if available, otherwise nullptr
            if let Some(d_p_neut_75pk) = self.buffer_pool.d_p_neut_time_series_75pk.as_ref() {
                builder.arg(d_p_neut_75pk);  // Real 75-PK P_neut series
            } else {
                builder.arg(&null_ptr);  // Fallback
            }

            // Current immunity levels: Use 75-PK if available
            if let Some(d_immunity_75) = self.buffer_pool.d_current_immunity_levels_75.as_ref() {
                builder.arg(d_immunity_75);  // Real 75-PK immunity levels
            } else {
                builder.arg(&null_ptr);  // Fallback
            }

            // PK parameters
            if let Some(d_pk_params) = self.buffer_pool.d_pk_params.as_ref() {
                builder.arg(d_pk_params);  // Real PK params
            } else {
                builder.arg(&null_ptr);  // Fallback
            }
            ...
            let mut d_freq_history = self.stream.alloc_zeros::<f32>(HISTORY_WEEKS)
                .map_err(|e| PrismError::gpu("mega_fused_batch", format!("Alloc freq_history: {}", e)))?;
            self.stream.memcpy_htod(&freq_hist_data, &mut d_freq_history)
            ...
            let mut d_all_frequencies = self.stream.alloc_zeros::<f32>(N_VARIANTS as usize)
                .map_err(|e| PrismError::gpu("mega_fused_batch", format!("Alloc all_frequencies: {}", e)))?;
            self.stream.memcpy_htod(&curr_freq_data, &mut d_all_frequencies)
            ...
            let mut d_all_gammas = self.stream.alloc_zeros::<f32>(N_VARIANTS as usize)
                .map_err(|e| PrismError::gpu("mega_fused_batch", format!("Alloc all_gammas: {}", e)))?;
            self.stream.memcpy_htod(&gamma_data, &mut d_all_gammas)
                .map_err(|e| PrismError::gpu("mega_fused_batch", format!("Copy all_gammas: {}", e)))?;

            builder.arg(&d_all_frequencies);  // Real buffer: current frequencies
            builder.arg(&d_all_gammas);       // Real buffer: fitness scores
            builder.arg(&d_freq_history);     // Real buffer: 35-week history
            builder.arg(&0i32);               // my_variant_idx (this variant's index = 0)
            builder.arg(&N_VARIANTS);         // n_variants (1 = single variant context)
            builder.arg(&0.5f32);             // days_since_vaccine_norm (6 months normalized)
            builder.arg(&0.5f32);             // days_since_wave_norm (90 days normalized)
            builder.arg(&0.0f32);             // immunity_derivative (stable)
            builder.arg(&0.6f32);             // immunity_source_ratio (60% vaccine-derived)
            builder.arg(&0.45f32);            // country_id_norm (Germany = 5/11 ≈ 0.45)

            // Params
            builder.arg(d_params);

            // eprintln!("[GPU DEBUG] Calling builder.launch()...");
    ...
    pub fn enhance_with_polycentric(
        ...
        self.stream.memcpy_htod(&escape_10d_flat, &mut d_escape_10d)
        ...
        let mut d_time_since_infection = self.stream.alloc_zeros::<f32>(time_since_infection.len().max(1))
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Alloc time_since_infection: {}", e)))?;
        self.stream.memcpy_htod(&time_since_infection, &mut d_time_since_infection)
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Upload time_since_infection: {}", e)))?;

        let mut d_freq_history = self.stream.alloc_zeros::<f32>(freq_history_flat.len().max(1))
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Alloc freq_history: {}", e)))?;
        self.stream.memcpy_htod(&freq_history_flat, &mut d_freq_history)
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Upload freq_history: {}", e)))?;

        let mut d_current_freq = self.stream.alloc_zeros::<f32>(current_freq.len().max(1))
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Alloc current_freq: {}", e)))?;
        ...
        let mut d_features = self.stream.alloc_zeros::<f32>(features_flat.len().max(1))
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Alloc features: {}", e)))?;
        self.stream.memcpy_htod(&features_flat, &mut d_features)
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Upload features: {}", e)))?;

        let mut d_residue_offsets = self.stream.alloc_zeros::<i32>(residue_offsets_flat.len().max(1))
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Alloc residue_offsets: {}", e)))?;
        self.stream.memcpy_htod(&residue_offsets_flat, &mut d_residue_offsets)
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Upload residue_offsets: {}", e)))?;

        let mut d_n_residues = self.stream.alloc_zeros::<i32>(n_residues_flat.len().max(1))
            .map_err(|e| PrismError::gpu("polycentric_enhance", format!("Alloc n_residues: {}", e)))?;
        self.stream.memcpy_htod(&n_residues_flat, &mut d_n_residues)
