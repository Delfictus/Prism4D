...
use cudarc::driver::{CudaContext, CudaStream, CudaFunction, CudaSlice, LaunchConfig, PushKernelArg};
...
impl PolycentricImmunityGpu {
    ...
    pub fn new(context: Arc<CudaContext>, ptx_path: &std::path::Path) -> Result<Self> {
        ...
        let mut cross_reactivity = stream.alloc_zeros::<f32>(cross_flat.len())?;
        stream.memcpy_htod(&cross_flat, &mut cross_reactivity)?;

        // Build PK parameter arrays (from VASIL specification)
        let tmax_values = build_pk_tmax();
        let thalf_values = build_pk_thalf();
        let mut pk_tmax = stream.alloc_zeros::<f32>(tmax_values.len())?;
        stream.memcpy_htod(&tmax_values, &mut pk_tmax)?;
        let mut pk_thalf = stream.alloc_zeros::<f32>(thalf_values.len())?;
        stream.memcpy_htod(&thalf_values, &mut pk_thalf)?;
    ...
    pub fn init_centers(
        ...
        // Upload to device
        let mut d_features = self.stream.alloc_zeros::<f32>(features.len())?;
        self.stream.memcpy_htod(features, &mut d_features)?;

        let mut d_labels = self.stream.alloc_zeros::<i32>(epitope_labels.len())?;
        self.stream.memcpy_htod(epitope_labels, &mut d_labels)?;
        ...
        let mut d_counts = self.stream.alloc_zeros::<i32>(counts.len())?;
        self.stream.memcpy_htod(&counts, &mut d_counts)?;
        ...
        unsafe {
            let mut builder = self.stream.launch_builder(&self.kernel_init_centers);
            builder.arg(&d_features);
            builder.arg(&d_labels);
            builder.arg(&d_counts);
            builder.arg(&n_samples_i32);
            builder.arg(&mut self.epitope_centers);
    ...
    pub fn process_batch(
        ...
        unsafe {
            let mut builder = self.stream.launch_builder(&self.kernel_main);
            builder.arg(features_packed);
            builder.arg(residue_offsets);
            builder.arg(n_residues);
            builder.arg(escape_10d);
            builder.arg(pk_immunity);
            builder.arg(time_since_infection);
            builder.arg(freq_history);
            builder.arg(current_freq);
