...
use cudarc::driver::{CudaContext, CudaSlice, CudaStream, DeviceRepr, LaunchConfig, ValidAsZeroBits};
...
impl AsyncCoordinator {
    ...
    pub fn new(context: Arc<CudaContext>) -> Result<Self> {
        // Create dedicated streams from default stream
        let default_stream = context.default_stream();
        let compute_stream = default_stream.fork()?;
        let upload_stream = default_stream.fork()?;
        let download_stream = default_stream.fork()?;
