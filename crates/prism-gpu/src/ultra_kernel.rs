...
impl UltraKernelGpu {
    ...
    pub fn new(
        ...
        for ptx_path in &ptx_paths {
            if std::path::Path::new(ptx_path).exists() {
                ...
                // load_module() already returns Arc<CudaModule>
                module = Some(context.load_module(Ptx::from_file(ptx_path))?);
                break;
            }
        }

        let module = module.context("Failed to load dr_whcr_ultra PTX module")?;

        // Get kernel functions
        let ultra_kernel = module.load_function("dr_whcr_ultra_kernel")?;
        let init_kernel = module.load_function("ultra_init_kernel")?;
        let finalize_kernel = module.load_function("ultra_finalize_kernel")?;
        let telemetry_kernel = module.load_function("ultra_telemetry_kernel")?;
    ...
    pub fn run(&self, iterations: usize) -> Result<(KernelTelemetry, Vec<i32>)> {
        ...
        for iter in 0..iterations {
            ...
            unsafe {
                self.stream
                    .launch_builder(&self.ultra_kernel)
