...
impl DendriticReservoir {
    ...
    pub fn new(ctx: Arc<CudaContext>, ptx_path: &str, config: DendriticConfig) -> Result<Self> {
        ...
        let fn_init = module.load_function("ve_swarm_init_reservoir")?;
        let fn_propagate = module.load_function("ve_swarm_dendritic_reservoir")?;
        let fn_attention = module.load_function("ve_swarm_compute_attention")?;
