...
impl ViralEvolutionFitnessGpu {
    ...
    fn compute_cross_neutralization(
        ...
        self.stream.memcpy_htod(&[self.params.clone()], &mut d_params)
    ...
    fn compute_unified_fitness(
        ...
        // Copy to GPU
        self.stream.memcpy_htod(fold_reduction, &mut d_fold_red)?;
        self.stream.memcpy_htod(ddg_fold, &mut d_ddg_fold)?;
        self.stream.memcpy_htod(ddg_bind, &mut d_ddg_bind)?;
        self.stream.memcpy_htod(expression, &mut d_expression)?;
        self.stream.memcpy_htod(&transmissibility, &mut d_transmit)?;
    ...
    pub fn predict_dynamics(
        ...
        self.stream.memcpy_htod(&current_freq, &mut d_freq)?;
