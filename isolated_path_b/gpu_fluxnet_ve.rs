...
impl GpuFluxNetVE {
    pub fn new(
        ...
        let base_ic50 = [0.85f32, 1.12, 0.93, 1.05, 0.98, 1.21, 0.89, 1.08, 0.95, 1.03];
        ...
        let mut trainer = Self {
            ...
            best_ic50: base_ic50,
            best_thresholds: [0.05, 0.03, 0.01, 0.0],
    ...
    fn initialize_configs(&mut self) -> Result<()> {
        let base_ic50 = [0.85f32, 1.12, 0.93, 1.05, 0.98, 1.21, 0.89, 1.08, 0.95, 1.03];
        let base_thresholds = [0.05f32, 0.03, 0.01, 0.0];
        ...
        for config_idx in 0..self.n_configs {
            ...
            for i in 0..N_IC50 {
                ic50_data.push(base_ic50[i] * (1.0 + perturbation * (i as f32 / N_IC50 as f32)));
            }
            for i in 0..N_THRESHOLDS {
                thresh_data.push(base_thresholds[i] * (1.0 + perturbation * 0.5));
            }
        }
    ...
    pub fn train_step(
        ...
        self.launch_compute_accuracy(
            &d_adjusted_gamma_min, &d_adjusted_gamma_max,
