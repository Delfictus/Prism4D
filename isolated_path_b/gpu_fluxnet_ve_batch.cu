...
#define N_ACTIONS 30  // 10 IC50 up + 10 IC50 down + 4 thresh up + 4 thresh down + noop + reset
#define N_STATES 64  // Match Rust: 4 acc bins × 4 excl bins × 4 total bins = 64
...
extern "C" __global__ void batch_gamma_adjust(
    ...
    // NO GAMMA ADJUSTMENT: The 75-PK immunity envelope values are physics-correct.
    // Additive IC50 adjustment degraded accuracy from 79% to 42% because it has
    // no physical basis. RL now optimizes THRESHOLD parameters only.
    // IC50 configs are unused but kept for API compatibility.
    (void)ic50_configs;
    (void)base_ic50;
    
    int out_idx = config_idx * n_samples + sample_idx;
    adjusted_gamma_min[out_idx] = base_min;
    adjusted_gamma_max[out_idx] = base_max;
    adjusted_gamma_mean[out_idx] = base_mean;
}
...
extern "C" __global__ void batch_compute_accuracy(
    ...
    const double* __restrict__ adjusted_gamma_mean,  // [n_configs, n_samples] - ADDED for mean-based decisions
    ...
    (void)threshold_configs;  // Other thresholds reserved for future use
    ...
    for (int sample_idx = blockIdx.x * blockDim.x + tid; 
        ...
        double g_mean = adjusted_gamma_mean[gamma_idx];
        
        // MEAN-BASED decision rule (VASIL methodology)
        // If gamma_mean > margin → predict RISE
        // If gamma_mean < -margin → predict FALL
        // Otherwise → UNDECIDED
        int predicted = 0;
        if (g_mean > confidence_margin) {
            predicted = 1;   // RISE
        } else if (g_mean < -confidence_margin) {
            predicted = -1;  // FALL
        } else {
            local_excluded++;
            continue;
